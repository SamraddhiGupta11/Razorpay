-- REVIVEAI DATABASE MIGRATION: 001_initial.sql
-- Baseline Checkout Abandonment and Recovery Schema
...
CREATE INDEX IF NOT EXISTS idx_customers_returning ON customers(is_returning);
...
CREATE INDEX IF NOT EXISTS idx_checkouts_customer ON checkout_sessions(customer_id);
CREATE INDEX IF NOT EXISTS idx_checkouts_cart_value ON checkout_sessions(cart_value);
CREATE INDEX IF NOT EXISTS idx_checkouts_started_at ON checkout_sessions(started_at);
CREATE INDEX IF NOT EXISTS idx_checkouts_device ON checkout_sessions(device);
CREATE INDEX IF NOT EXISTS idx_checkouts_payment ON checkout_sessions(payment_method);
...
CREATE INDEX IF NOT EXISTS idx_abandoned_checkout ON abandoned_carts(checkout_id);
CREATE INDEX IF NOT EXISTS idx_abandoned_reason ON abandoned_carts(abandonment_reason);
CREATE INDEX IF NOT EXISTS idx_abandoned_status ON abandoned_carts(status);
CREATE INDEX IF NOT EXISTS idx_abandoned_detected_at ON abandoned_carts(detected_at);
...
CREATE TABLE IF NOT EXISTS interventions (
    ...
    action VARCHAR(30) NOT NULL CHECK (action IN ('NO_ACTION', 'PERSONALIZED_REMINDER', 'PAYMENT_ASSISTANCE', 'FREE_SHIPPING', 'DISCOUNT_5', 'DISCOUNT_10', 'TECH_SUPPORT', 'VIP_CONCIERGE')),
...
CREATE INDEX IF NOT EXISTS idx_interventions_cart ON interventions(abandoned_cart_id);
CREATE INDEX IF NOT EXISTS idx_interventions_channel ON interventions(channel);
...
CREATE INDEX IF NOT EXISTS idx_conversions_intervention ON conversions(intervention_id);
...
CREATE INDEX IF NOT EXISTS idx_predictions_checkout ON model_predictions(checkout_id);
